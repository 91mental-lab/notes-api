...
@patch('SecureNotesAPI.src.auth.SECRET_KEY', TEST_SECRET_KEY)
@patch("SecureNotesAPI.src.auth.ALGORITHM", ALGORITHM)
...
async def test_get_current_user_success(mock_db_session, mock_user):
    with patch('SecureNotesAPI.src.auth.jwt.decode', return_value = {"sub": "mocker", "exp": 9999999999}) as mock_jwt_decode:
...
async def test_get_current_user_invalid_token():
    with patch('SecureNotesAPI.src.auth.jwt.decode', side_effect=JWTError):
...
async def test_get_current_user_no_username_in_token():
    with patch('SecureNotesAPI.src.auth.jwt.decode', return_value = {"foo": "bar"}):
        with pytest.raises(HTTPException) as exc_info:
...
async def test_get_current_user_user_not_found(mock_db_session):
    with patch('SecureNotesAPI.src.auth.jwt.decode', return_value = {"sub": "mocker", "exp": 9999999999}):
